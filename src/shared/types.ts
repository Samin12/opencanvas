...
export type CanvasDiagramLayoutKind =
  | "grid"
  | "flow"
  | "radial"
  | "sequence"
...
export interface CollaboratorApi {
  ...
  onCanvasPinch: (
    listener: (payload: CanvasPinchPayload) => void,
  ) => () => void;
  onAppUpdateState: (listener: (state: AppUpdateState) => void) => () => void;
  readOfficeViewerBootstrap: (
    force?: boolean,
