...
export function TerminalPane({ cwd, sessionId }: TerminalPaneProps) {
  ...
  return (
    <div
      className="terminal-pane flex h-full flex-col overflow-hidden rounded-[8px] border border-slate-200 bg-[#fffdfa]"
      data-scroll-lock="true"
      onMouseDown={() => terminalRef.current?.focus()}
      onPointerDown={(event) => event.stopPropagation()}
    >
      <div className="flex items-center justify-between border-b border-slate-200 px-3 py-1 text-[10px] uppercase tracking-[0.16em] text-slate-400">
        <span>Terminal</span>
        <span>
          {status === 'connecting'
            ? 'Connecting'
