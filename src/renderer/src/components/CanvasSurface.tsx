...
import {
  ...
  type TerminalCardTransferPayload,
...
const NOTE_TILE_COLLAPSED_HEIGHT = 92;
...
const MARKDOWN_SHORTCUT_KEY = IS_MAC_PLATFORM ? "Cmd+N" : "Ctrl+N";
...
const NEXT_BOARD_ITEM_SHORTCUT_KEY = IS_MAC_PLATFORM
  ? "Option+Shift+→"
  : "Alt+Shift+→";
const PREVIOUS_BOARD_ITEM_SHORTCUT_KEY = IS_MAC_PLATFORM
  ? "Option+Shift+←"
  : "Alt+Shift+←";
...
const SHORTCUT_ITEMS: Array<{
  ...
  {
    action: "terminal-codex",
    key: CREATE_CODEX_TERMINAL_SHORTCUT_KEY,
...
const PREVIEW_SHORTCUT_ITEMS: Array<{ key: string; label: string }> = [
  {
    key: PLACE_ON_CANVAS_SHORTCUT_KEY,
    label: "Place preview and return to canvas",
  },
...
const BOARD_NAVIGATION_SHORTCUT_ITEMS: Array<{ key: string; label: string }> = [
  ...
  {
    key: INTERACT_WITH_SELECTED_ITEM_SHORTCUT_KEY,
    label: "Interact with selection",
  },
];
...
function shortcutGuideMarkdown() {
  return [
    ...
    ...shortcutGuideSection(
      "Board Navigation",
      BOARD_NAVIGATION_SHORTCUT_ITEMS,
    ),
    "",
    ...shortcutGuideSection(
      "Canvas Shortcuts",
...
function scrollLockElementFromTarget(
  ...
  while (element) {
    if (
      ...
      element.matches(SCROLL_LOCK_SELECTOR)
    ) {
      return element;
    }
...
function elementsShareCaptureChain(
...
function firstScrollableDescendant(root: HTMLElement): HTMLElement | null {
  ...
  for (const element of descendants) {
    ...
    const canScrollX =
      isScrollableOverflow(styles.overflowX) &&
...
function canScrollElementForDelta(
  element: HTMLElement,
  deltaX: number,
  deltaY: number,
) {
...
function hasCollaboratorFilePayload(dataTransfer: DataTransfer | null) {
  return Boolean(
    dataTransfer &&
    Array.from(dataTransfer.types).includes(COLLABORATOR_FILE_MIME),
  );
}
...
function hasCollaboratorTerminalCardPayload(dataTransfer: DataTransfer | null) {
  return Boolean(
    dataTransfer &&
    Array.from(dataTransfer.types).includes(COLLABORATOR_TERMINAL_CARD_MIME),
  );
}
...
function isImportableImageFile(file: File) {
  ...
  return Array.from(IMAGE_FILE_EXTENSIONS).some((extension) =>
...
function isImportableAssetFile(file: File) {
  ...
  return (
    Array.from(VIDEO_FILE_EXTENSIONS).some((extension) =>
      lowerName.endsWith(extension),
    ) ||
    Array.from(PDF_FILE_EXTENSIONS).some((extension) =>
      lowerName.endsWith(extension),
    ) ||
    Array.from(SPREADSHEET_FILE_EXTENSIONS).some((extension) =>
      lowerName.endsWith(extension),
    ) ||
    Array.from(PRESENTATION_FILE_EXTENSIONS).some((extension) =>
      lowerName.endsWith(extension),
    )
  );
}
...
function isRenderableImportedFile(node: FileTreeNode) {
  ...
  return (
    RENDERABLE_CODE_FILE_EXTENSIONS.has(extension) ||
    RENDERABLE_CODE_FILE_NAMES.has(lowerName)
  );
}
...
function cameraToViewport(camera: {
  x: number;
  y: number;
  z: number;
...
function viewportChanged(
  left: CanvasState["viewport"],
  right: CanvasState["viewport"],
) {
  return (
...
function terminalProviderForTile(
  tile: CanvasTile | null | undefined,
): TerminalProvider {
...
function titleForTerminal(tiles: CanvasTile[], provider: TerminalProvider) {
  const count =
    tiles.filter(
      (tile) =>
        tile.type === "term" && terminalProviderForTile(tile) === provider,
    ).length + 1;
  if (provider === "codex") {
    return `Codex ${count}`;
  }
...
function minTileHeightForTile(tile: CanvasTile) {
  ...
  return tile.noteCollapsed ? NOTE_TILE_COLLAPSED_HEIGHT : NOTE_TILE_MIN_HEIGHT;
}
...
function expandedHeightForNote(tile: CanvasTile) {
  if (tile.type !== "note") {
    return tile.height;
  }

  const storedExpandedHeight =
    typeof tile.noteExpandedHeight === "number" &&
    Number.isFinite(tile.noteExpandedHeight)
      ? tile.noteExpandedHeight
      : tile.height;

  return snap(Math.max(NOTE_TILE_MIN_HEIGHT, storedExpandedHeight));
}

function createEmbedTile(
  url: string,
  x: number,
  y: number,
  zIndex: number,
...
function contextResourcePromptLines(tile: CanvasTile) {
  const lines = [
    `- ${contextResourceTarget(tile)} (${contextResourceLabel(tile)})`,
  ];
...
function isMarkdownNotePath(targetPath: string | null | undefined) {
  return (
    typeof targetPath === "string" && /\.(?:md|mdx|markdown)$/i.test(targetPath)
  );
}
...
>(function CanvasSurface(
  ...
  const [hoveredConnection, setHoveredConnection] =
    useState<HoveredConnectionState | null>(null);
  const [hoveredTileId, setHoveredTileId] = useState<string | null>(null);
  const [linkSourceTileId, setLinkSourceTileId] = useState<string | null>(null);
  const [selectedCanvasNoteId, setSelectedCanvasNoteId] = useState<
    string | null
  >(null);
  const [selectedTileId, setSelectedTileId] = useState<string | null>(null);
  const [selectedTileIds, setSelectedTileIds] = useState<string[]>([]);
  const [tileSelectionBox, setTileSelectionBox] =
    useState<TileSelectionBox | null>(null);
  const [focusedTerminal, setFocusedTerminal] = useState<{
    mode: TerminalUiMode;
    tileId: string;
  } | null>(null);
  const [tileRefreshTokens, setTileRefreshTokens] = useState<
  ...
  function setTileSelection(
    tileIds: string[],
    options?: {
      preserveFocusedTerminal?: boolean;
    ...
    const nextPrimaryTileId =
      nextSelectedTileIds.length === 1
        ...
        : options?.primaryTileId &&
            nextSelectedTileIds.includes(options.primaryTileId)
          ? options.primaryTileId
          : null;

    setSelectedTileIds((current) =>
      sameTileIdList(current, nextSelectedTileIds)
        ? current
        : nextSelectedTileIds,
    );
    setSelectedTileId((current) =>
      current === nextPrimaryTileId ? current : nextPrimaryTileId,
    );

    if (!options?.preserveFocusedTerminal) {
      setFocusedTerminal((current) =>
        current &&
        nextPrimaryTileId === current.tileId &&
        nextSelectedTileIds.length === 1
          ? current
          : null,
      );
  ...
  function bringTilesToFront(tileIds: string[]) {
    ...
    const nextZIndexByTileId = new Map(
      orderedTileIds.map((tileId, index) => [tileId, nextZIndexStart + index]),
    );
  ...
  async function handleNotePrimaryHeadingChange(
    tile: CanvasTile,
    heading: string | null,
  ) {
    if (
  ...
  function isContextSourceTile(tile: CanvasTile | null): tile is CanvasTile {
    return Boolean(
      tile && tile.type !== "term" && (tile.filePath || tile.embedUrl),
    );
  }
  ...
  function selectedStickyNoteFromEditor(editor: Editor) {
    ...
    const bounds =
      ...
      editor.getShapePageBounds(noteShape.id);
  ...
  function selectedEditableRichTextShapeFromEditor(editor: Editor) {
    ...
    if (
      !selectedShape ||
      (selectedShape.type !== "note" && selectedShape.type !== "text")
    ) {
      return null;
    }
  ...
  function traversableBoardItems(): TraversableBoardItem[] {
    const tileItems: TraversableBoardItem[] = stateRef.current.tiles.map(
      (tile) => ({
        kind: "tile",
        id: tile.id,
        bounds: {
  ...
  function currentTraversableBoardItem(items: TraversableBoardItem[]) {
    if (selectedTileId && selectedTileIds.length === 1) {
      return (
        items.find(
          (item) => item.kind === "tile" && item.id === selectedTileId,
        ) ?? null
      );
    }
    ...
    return (
      items.find(
        (item) => item.kind === "shape" && item.id === primaryShapeId,
      ) ?? null
    );
  }

  function centerViewportOnBounds(bounds: {
    height: number;
    width: number;
    x: number;
    y: number;
  ...
  function startEditingSelectedRichTextShape(options?: {
    selectAll?: boolean;
  }) {
    const editor = editorRef.current;
    ...
    return currentPageShapes
      ...
      .flatMap((shape) => {
        ...
        const hasFrameChildren = currentPageShapes.some(
          (candidate) =>
            candidate.parentId === shape.id && candidate.type !== "frame",
        );
  ...
  function expandFrameBoundsForTile(
    bounds: FrameBoundsSnapshot,
    tile: CanvasTile,
  ): FrameBoundsSnapshot {
    const nextLeft = Math.min(bounds.x, tile.x - GROUP_FRAME_INSET);
    const nextTop = Math.min(
      bounds.y,
      tile.y - GROUP_FRAME_HEADER_HEIGHT - GROUP_FRAME_INSET,
    );
    const nextRight = Math.max(
      bounds.x + bounds.width,
      tile.x + tile.width + GROUP_FRAME_INSET,
    );
    const nextBottom = Math.max(
      bounds.y + bounds.height,
      tile.y + tile.height + GROUP_FRAME_INSET,
    );
  ...
  function expandGroupsForTileIds(tileIds: string[]) {
    ...
    const frameUpdates: FrameShapeUpdate[] = Array.from(
      nextBounds.entries(),
  ...
  function contextTilesForTerminal(tile: CanvasTile) {
    ...
    const groupedTileIds = new Set(
      contextGroupsForTerminal(tile).flatMap((group) =>
        group.containedTiles.map((groupTile) => groupTile.id),
      ),
    );
  ...
  function resolvedContextTilesForTerminal(tile: CanvasTile) {
    ...
    return Array.from(
      new Map(
        [
          ...contextGroupsForTerminal(tile).flatMap(
            (group) => group.containedTiles,
          ),
          ...contextTilesForTerminal(tile),
        ]
          .filter(isContextSourceTile)
          .map(
            (contextTile) =>
              [contextResourceIdentity(contextTile), contextTile] as const,
          ),
      ).values(),
    );
  }
  ...
  function terminalConnectorIdFromTarget(target: EventTarget | null) {
    ...
    const connector = target.closest<HTMLElement>(
      "[data-terminal-connector-id]",
    );
  ...
  function showHoveredTileConnection(
    sourceTile: CanvasTile,
  ...
  function showHoveredGroupConnection(
    sourceGroup: FrameContextGroup,
    terminalTile: CanvasTile,
  ) {
  ...
  function isSameConnectionHoverTarget(
    target: EventTarget | null,
    ...
    return (
      target
        .closest("[data-terminal-connection-key]")
        ?.getAttribute("data-terminal-connection-key") === connectionKey
    );
  }
    ...
    setHoveredConnection((current) =>
      ...
      (current.sourceKind === "group"
        ...
        : terminalConnectionKey(
            current.sourceTileId,
            current.terminalTileId,
          )) === connectionKey
        ? null
        : current,
    );
  ...
  function beginGroupConnectionDrag(
    groupId: string,
  ...
  function moveFrameBundle(
    ...
    const editor = editorRef.current;
    const nextFrameX = options?.snapToGrid
      ? snap(interaction.frameStartX + deltaX)
      : interaction.frameStartX + deltaX;
    const nextFrameY = options?.snapToGrid
      ? snap(interaction.frameStartY + deltaY)
      : interaction.frameStartY + deltaY;
  ...
  function buildTerminalContextPrompt(
    ...
    const hasMarkdownContext = contextTiles.some(
      (tile) => tile.type === "note",
    );
    const hasPdfContext = contextTiles.some((tile) => tile.type === "pdf");
    const hasSpreadsheetContext = contextTiles.some(
      (tile) => tile.type === "spreadsheet",
    );
    const hasPresentationContext = contextTiles.some(
      (tile) => tile.type === "presentation",
    );
    const hasVideoContext = contextTiles.some((tile) => tile.type === "video");
    const hasLinkContext = contextTiles.some(
      (tile) => tile.type === "embed" || Boolean(tile.embedUrl),
    );
    ...
    if (contextGroups.length > 0) {
      ...
      contextGroups.forEach((group) => {
        ...
        group.containedTiles.forEach((groupTile) => {
          contextResourcePromptLines(groupTile).forEach((line) =>
            lines.push(`  ${line}`),
          );
        });
      });
    ...
    if (looseContextTiles.length > 0) {
      ...
      looseContextTiles.forEach((contextTile) => {
        contextResourcePromptLines(contextTile).forEach((line) =>
          lines.push(line),
        );
      });
    }
    ...
    if (hasImageContext) {
      lines.push(
        "For each path marked (image), inspect that image before responding.",
      );
    }

    if (hasPdfContext) {
      lines.push(
        "For each path marked (pdf), read that PDF before responding.",
      );
    }
    ...
    lines.push(
      "Keep all of these resources in working context for follow-up questions.",
    );

    return lines.join("\n");
  }

  function attachContextTileIds(
    ...
    if (
      validSourceTileIds.length === 0 ||
      !terminalTile ||
      terminalTile.type !== "term"
    ) {
      return;
    }
  ...
  function attachContextGroup(groupId: string, terminalTileId: string) {
    ...
    const nextContextGroupIds = Array.from(
      new Set([...(terminalTile.contextGroupIds ?? []), groupId]),
    );
  ...
  async function createMarkdownCardForTerminal(
    ...
    if (bounds) {
      ...
      const maxX = Math.max(
        minX,
        bounds.right - draftTile.width - viewportMargin,
      );
      const minY = bounds.top + viewportMargin;
      const maxY = Math.max(
        minY,
        bounds.bottom - draftTile.height - viewportMargin,
      );
  ...
  function pushDeletedTileUndoEntry(entry: DeletedTileUndoEntry) {
    deletedTileUndoStackRef.current = [
      ...deletedTileUndoStackRef.current.slice(-19),
      entry,
    ];
  }
  ...
  function restoreDeletedTileUndoEntry(entry: DeletedTileUndoEntry) {
    ...
    const existingTilesById = new Map(
      stateRef.current.tiles.map((tile) => [tile.id, tile]),
    );
    ...
    const contextRestorationsByTileId = new Map(
      entry.terminalContextRestorations.map((restoration) => [
        restoration.tileId,
        restoration.contextTileIds,
      ]),
    );
    ...
    const nextTiles = [...existingTilesById.values()]
      .map((tile) => {
        ...
        previousContextTileIds.forEach((contextTileId) => {
          if (
            (currentContextTileIdSet.has(contextTileId) ||
              restoredTileIdSet.has(contextTileId)) &&
  ...
  function undoLastDeletedTiles() {
    ...
    deletedTileUndoStackRef.current = deletedTileUndoStackRef.current.slice(
      0,
      -1,
    );
  ...
  function handleNoteContentHeightChange(
    tileId: string,
  ...
  function toggleNoteCollapsed(tileId: string) {
    updateTile(
      tileId,
      (currentTile) => {
        if (currentTile.type !== "note") {
          return currentTile;
        }

        if (currentTile.noteCollapsed) {
          return {
            ...currentTile,
            height: expandedHeightForNote(currentTile),
            noteCollapsed: false,
          };
        }
        ...
        return {
          ...currentTile,
          height: NOTE_TILE_COLLAPSED_HEIGHT,
          noteCollapsed: true,
          noteExpandedHeight: Math.max(
            currentTile.height,
  ...
  function showZoom(nextZoom: number) {
    ...
    window.clearTimeout(
      (showZoom as typeof showZoom & { timer?: number }).timer,
    );
    (showZoom as typeof showZoom & { timer?: number }).timer =
      window.setTimeout(() => {
  ...
  function pagePointFromClient(clientX: number, clientY: number) {
    ...
    return {
      x:
        (clientX - rect.left - stateRef.current.viewport.panX) /
        stateRef.current.viewport.zoom,
      y:
        (clientY - rect.top - stateRef.current.viewport.panY) /
        stateRef.current.viewport.zoom,
    };
  }
  ...
  function resolveWheelGestureOwner(
    ...
    if (
      scrollLockElement &&
      elementsShareCaptureChain(scrollLockElement, activeScrollCaptureElement)
    ) {
      const scrollableElement = scrollableElementFromTarget(target, container);

      if (
        scrollableElement &&
        canScrollElementForDelta(scrollableElement, deltaX, deltaY)
      ) {
        return {
          kind: "element",
      ...
      return {
        ...
        element:
          scrollableElement ??
          firstScrollableDescendant(scrollLockElement) ??
          scrollLockElement,
      };
    }
  ...
  function applyWheelGestureOwner(
    owner: WheelGestureOwner,
    deltaX: number,
    deltaY: number,
  ) {
  ...
  function zoomAt(clientX: number, clientY: number, nextZoom: number) {
    ...
    const worldX =
      (clientX - rect.left - stateRef.current.viewport.panX) / previousZoom;
    const worldY =
      (clientY - rect.top - stateRef.current.viewport.panY) / previousZoom;
  ...
  function updatePinch(payload: {
    ...
    if (
      ...
      const normalizedDelta =
        Math.abs(payload.delta) > 2 ? payload.delta * 0.01 : payload.delta;
  ...
  function centerWorldPosition(spread = 0) {
    ...
    return {
      x:
        (rect.width / 2 - stateRef.current.viewport.panX) /
          stateRef.current.viewport.zoom +
        offset.x,
      y:
        (rect.height / 2 - stateRef.current.viewport.panY) /
          stateRef.current.viewport.zoom +
        offset.y,
    };
  }
  ...
  function preferredTerminalFrame(preferredX: number, preferredY: number) {
    ...
    const maxWidth = bounds
      ? Math.max(
          MIN_TILE_WIDTH,
          bounds.right - bounds.left - viewportMargin * 2,
        )
      : DEFAULT_TERMINAL_WIDTH;
    const maxHeight = bounds
      ? Math.max(
          MIN_TILE_HEIGHT,
          bounds.bottom - bounds.top - viewportMargin * 2,
        )
  ...
  async function importExcalidrawContent(
    content: unknown,
    ...
    const frameId =
      `shape:${Date.now()}-${Math.random().toString(36).slice(2, 8)}` as TLFrameShape["id"];
    const frameInset = 40;
  ...
  function spawnFileTileAtClientPoint(
    file: FileTreeNode,
    clientX: number,
    clientY: number,
  ) {
  ...
  async function createMarkdownNoteAtClientPoint(
    clientX: number,
    clientY: number,
  ) {
    const createdNode = await onCreateMarkdownCard({
      initialContent: "",
      targetDirectoryPath:
        importTargetDirectoryPath ?? activeWorkspacePath ?? undefined,
    });
  ...
  function focusSelectedTileEditor() {
    ...
    const documentEditTarget = tileRoot?.querySelector<HTMLElement>(
    ...
    documentEditTarget.dispatchEvent(
      new Event("collaborator:focus-document-editor"),
    );
    return true;
  }

  function spawnEmbedTileAtClientPoint(
    url: string,
    clientX: number,
    clientY: number,
  ) {
  ...
  function createEditableTextAtViewportCenter() {
    ...
    const textShapeId =
      `shape:${Date.now()}-${Math.random().toString(36).slice(2, 8)}` as TLTextShape["id"];
  ...
  async function convertSelectedStickyNoteToMarkdown() {
    ...
    try {
      const convertedNode = await onConvertStickyNoteToMarkdown(
        selectedNote.text,
      );
  ...
  function handleBoardMount(editor: Editor) {
    ...
    if (
      stateRef.current.boardSnapshot &&
      typeof stateRef.current.boardSnapshot === "object"
    ) {
      try {
        editor.loadSnapshot(
          stateRef.current.boardSnapshot as Parameters<
            Editor["loadSnapshot"]
          >[0],
        );
      } catch {
    ...
    setSelectedCanvasNoteId(
      selectedStickyNoteFromEditor(editor)?.shape.id ?? null,
    );
    onBoardReady?.();
    ...
    const handleCreatedShapes = (records: TLRecord[]) => {
      ...
      const frameShapeUpdates: FrameShapeUpdate[] = records.flatMap(
        (record) => {
              ...
              props: {
                ...
                name: (() => {
                  if (nextGeneratedGroupNumber === null) {
                    ...
                    nextGeneratedGroupNumber = match
    ...
    const detachStoreListener = editor.store.listen(
      () => {
        const resolvedBoardTool = boardToolFromEditorToolId(
        ...
        setSelectedCanvasNoteId((current) =>
          current === (selectedStickyNote?.shape.id ?? null)
            ? current
            : (selectedStickyNote?.shape.id ?? null),
        );
        ...
        if (
          Math.abs(currentViewport.zoom - nextViewport.zoom) > CAMERA_EPSILON
        ) {
  ...
  useEffect(() => {
    ...
    if (
      !selectedTile ||
      selectedTile.type !== "term" ||
      selectedTile.id !== focusedTerminal.tileId
    ) {
      setFocusedTerminal(null);
    }
  ...
  useEffect(() => {
    ...
    const nextSelectedTileIds = selectedTileIds.filter((tileId) =>
      existingTileIdSet.has(tileId),
    );
  ...
  useEffect(() => {
    ...
    function handleGlobalWheel(event: WheelEvent) {
      ...
      if (!event.ctrlKey && !event.metaKey) {
        ...
        const owner = shouldPreferResolvedWheelOwner(
          activeWheelOwner,
          resolvedOwner,
        )
          ? resolvedOwner
      ...
      const zoomDelta =
        -normalizeWheelDelta(event.deltaY, event.deltaMode) * 0.01;
    ...
    window.addEventListener(
      "gesturestart",
      handleGlobalGestureStart as EventListener,
      {
        passive: false,
        capture: true,
      },
    ...
    window.addEventListener(
      "gesturechange",
      handleGlobalGestureChange as EventListener,
      {
        passive: false,
        capture: true,
      },
    ...
    window.addEventListener(
      "gestureend",
      handleGlobalGestureEnd as EventListener,
      {
        capture: true,
      },
    );
  ...
  useEffect(() => {
    const detach = window.collaborator.onCanvasPinch(
      (payload: CanvasPinchPayload) => {
  ...
  useEffect(() => {
    function syncBoardKeyboardFocus(target: EventTarget | null) {
      ...
      const targetInsideCanvas = Boolean(
        container && targetNode && container.contains(targetNode),
      );
      const boardShouldOwnFocus =
        !shortcutsSuspended &&
        targetInsideCanvas &&
        !keyboardShortcutsBlocked(target);
    ...
    function handlePointerContext(event: PointerEvent) {
      ...
      lastPointerScrollActivationAtRef.current = targetInsideCanvas
        ? Date.now()
        : 0;
      activeScrollCaptureElementRef.current =
        container && targetInsideCanvas
          ? wheelCaptureElementFromTarget(event.target, container)
          : null;
    ...
    function handleFocusIn(event: FocusEvent) {
      ...
      if (
        container &&
        targetInsideCanvas &&
        Date.now() - lastPointerScrollActivationAtRef.current <=
          POINTER_SCROLL_ACTIVATION_WINDOW_MS
      ) {
        activeScrollCaptureElementRef.current = wheelCaptureElementFromTarget(
          event.target,
          container,
        );
      }
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (isUndoShortcut) {
        if (
          ...
          keyboardShortcutsBlocked(event.target)
        ) {
          return;
        }
        ...
        updateState({
          ...
          tiles: stateRef.current.tiles.map((tile) =>
            interaction.tileStarts.some((start) => start.id === tile.id)
              ? (() => {
                  const start = interaction.tileStarts.find(
      ...
      if (dragConnectionRef.current) {
        ...
        if (terminalTileId) {
          if (dragConnectionRef.current.sourceKind === "tile") {
            attachContextTile(
              dragConnectionRef.current.sourceTileId,
              terminalTileId,
            );
          } else {
            attachContextGroup(
              dragConnectionRef.current.sourceGroupId,
              terminalTileId,
            );
          }
        } else {
  ...
  function beginTileDrag(
    ...
    const draggedTileIds =
      selectedTileIds.includes(tile.id) && selectedTileIds.length > 1
        ? selectedTileIds
        : [tile.id];
  ...
  function deleteTiles(tileIds: string[]) {
    ...
    const deletedTiles = stateRef.current.tiles.filter((tile) =>
      deletedTileIdSet.has(tile.id),
    );
    ...
    pushDeletedTileUndoEntry({
      terminalContextRestorations: stateRef.current.tiles.flatMap((tile) => {
        ...
        return contextTileIds.some((contextTileId) =>
          deletedTileIdSet.has(contextTileId),
        )
          ? [
              {
                contextTileIds,
    ...
    setHoveredConnection((current) =>
      ...
      (deletedTileIdSet.has(current.terminalTileId) ||
        (current.sourceKind === "tile" &&
          deletedTileIdSet.has(current.sourceTileId)))
        ? null
        : current,
    );

    setHoveredTileId((current) =>
      current && deletedTileIdSet.has(current) ? null : current,
    );
  }
  ...
  function createTerminalAt(
    clientX: number,
    clientY: number,
    provider: TerminalProvider = "claude",
  ) {
    const point = pagePointFromClient(clientX, clientY);
  ...
  function handleDragOverCapture(event: ReactDragEvent<HTMLDivElement>) {
    ...
    if (!activeWorkspacePath || !hasExternalPathPayload(event.dataTransfer)) {
      if (
        !activeWorkspacePath ||
        droppedImportableAssetFiles(event.dataTransfer).length === 0
      ) {
        return;
      }
  ...
  function handleDropCapture(event: ReactDragEvent<HTMLDivElement>) {
    ...
    if (droppedUrl) {
      event.preventDefault();
      event.stopPropagation();
      spawnEmbedTileAtClientPoint(
        droppedUrl.canonicalUrl,
        event.clientX,
        event.clientY,
      );
      return;
    ...
    const externalDownload = externalDownloadFromDataTransfer(
      event.dataTransfer,
    );
    ...
    if (externalPaths.length > 0 || externalDownload) {
      ...
      void (async () => {
        ...
        const importedFiles = collectRenderableImportedFiles(
          await importedNodes,
        );
  ...
  const linkSourceTile = isContextSourceTile(activeLinkSourceTile)
    ? activeLinkSourceTile
    : null;
  ...
  const drawTheme = darkMode
    ? DefaultColorThemePalette.darkMode
    : DefaultColorThemePalette.lightMode;
  const visibleDrawColorItems = DRAW_COLOR_ITEMS.filter(
    (item) => darkMode || !item.hideInLightMode,
  );
  const activeDrawColorItem =
    visibleDrawColorItems.find((item) => item.color === drawColor) ??
    visibleDrawColorItems[0] ??
    null;
  const activeDrawColorLabel = activeDrawColorItem
    ? drawColorLabel(activeDrawColorItem, darkMode)
    : "Ink";
  const activeDrawSizeItem =
    DRAW_SIZE_ITEMS.find((item) => item.size === drawSize) ??
    DRAW_SIZE_ITEMS[1] ??
    null;
  ...
  const hoveredConnectionKey = hoveredConnection
    ? hoveredConnection.sourceKind === "group"
      ...
      : terminalConnectionKey(
          hoveredConnection.sourceTileId,
          hoveredConnection.terminalTileId,
        )
    : null;
  const hoveredConnectionSourceTile =
    hoveredConnection?.sourceKind === "tile"
      ? tileById(hoveredConnection.sourceTileId)
      : null;
  const hoveredConnectionSourceGroup =
    hoveredConnection?.sourceKind === "group"
      ? (frameGroups.find(
          (group) => group.id === hoveredConnection.sourceGroupId,
        ) ?? null)
      : null;
  const hoveredConnectionTerminalTile = hoveredConnection
    ? tileById(hoveredConnection.terminalTileId)
    : null;
  const selectedTileIdSet = new Set(selectedTileIds);
  const multiSelectedTiles = selectedTileIds.length > 1;
  const activeTileSelectionBounds = tileSelectionBox
    ? selectionBoxBounds(tileSelectionBox)
    : null;
  ...
  function renderTerminalTile(tile: CanvasTile) {
    ...
    const hasLinkedContext =
      contextGroups.length > 0 || contextTiles.length > 0;
    const terminalContextPrompt = buildTerminalContextPrompt(
      tile,
    ...
    const isTilePrimarySelected =
      isTileSelected && !multiSelectedTiles && selectedTileId === tile.id;
      ...
      >
        ...
        >
          <TerminalPane
            ...
            onCreateMarkdownCard={(options) =>
              createMarkdownCardForTerminal(tile.id, options)
            }
            ...
            <div
              ...
              style={{
                ...
                left: handle.includes("w")
                  ? metrics.connectorInset / 2
                  : handle.includes("e")
                    ? undefined
                    : metrics.resizeInset,
                right: handle.includes("e")
                  ? -metrics.resizeSize / 3
                  : handle.includes("w")
                    ? undefined
                    : metrics.resizeInset,
  ...
  return (
    <div
      ...
      onPointerDownCapture={(event) => {
        ...
        if (
          target.closest('[data-tile-root="true"]') ||
          target.closest('[data-canvas-ui="true"]')
        ) {
          return;
        }

        if (
          activeBoardTool === "select" &&
          isTileSelectionBackgroundTarget(target)
        ) {
    ...
    >
      {linkSourceTile ? (
        <div className="pointer-events-none absolute left-3 top-3 z-[210] rounded-[var(--radius-control)] border border-[color:var(--line)] bg-[color:var(--surface-overlay)] px-3 py-1.5 text-[11px] font-medium tracking-[0.12em] text-[var(--text-dim)] backdrop-blur">
          Drag{" "}
          <span className="text-[var(--text)]">{linkSourceTile.title}</span>{" "}
          into a terminal dot, or press Esc to cancel.
        </div>
      ) : null}
      ...
      >
        {activeTileSelectionBounds ? (
          <div
            ...
            style={{
              ...
              width: Math.max(
                1,
                activeTileSelectionBounds.right -
                  activeTileSelectionBounds.left,
              ),
              height: Math.max(
                1,
                activeTileSelectionBounds.bottom -
                  activeTileSelectionBounds.top,
              ),
            }}
          />
        ) : null}
          ...
          {terminalConnections.map((connection) => {
            ...
            const connectionKey =
              connection.sourceKind === "group"
                ...
                : terminalConnectionKey(
                    connection.sourceTile.id,
                    connection.terminalTile.id,
                  );
            const isHoveredConnection =
              connection.sourceKind === "tile" &&
              ...
              return (
                <g key={connectionKey}>
                  <path
                    ...
                    onPointerEnter={() =>
                      showHoveredGroupConnection(
                        connection.sourceGroup,
                        connection.terminalTile,
                      )
                    }
            ...
            return (
              <g key={connectionKey}>
                ...
                <path
                  ...
                  data-terminal-connection-key={connectionKey}
                  onPointerEnter={() =>
                    showHoveredTileConnection(sourceTile, terminalTile)
                  }
                  onPointerLeave={(event) =>
                    clearHoveredConnection(connectionKey, event.relatedTarget)
                  }
            ...
            ? (() => {
                const start =
                  dragConnection.sourceKind === "tile"
                    ? (() => {
                        const sourceTile = tileById(
                          dragConnection.sourceTileId,
                        );
                        return isContextSourceTile(sourceTile)
                          ? connectorCenterForTile(sourceTile)
                          : null;
                      })()
                    : (() => {
                        ...
                        return sourceGroup
                          ? connectorCenterForGroup(sourceGroup)
                          : null;
                      })();
        ...
        {frameGroups.map((group) => {
          ...
          const isDraggingGroup =
            dragConnection?.sourceKind === "group" &&
            dragConnection.sourceGroupId === group.id;
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                isDraggingGroup ? "scale-[1.18]" : "hover:scale-110",
              )}
              style={{
                left: connector.x - 12,
                top: connector.y,
              ...
              onPointerDown={(event) =>
                beginGroupConnectionDrag(group.id, event)
              }
            >
        ...
        {contentTiles.map((tile) =>
          (() => {
            const contextTiles =
              tile.type === "term" ? contextTilesForTerminal(tile) : [];
            const contextGroups =
              tile.type === "term" ? contextGroupsForTerminal(tile) : [];
            const hasLinkedContext =
              contextGroups.length > 0 || contextTiles.length > 0;
            ...
            const isMarkdownTile =
              tileFileKind === "note" && isMarkdownNotePath(tile.filePath);
            const isRenamingThisTile = renamingTileId === tile.id;
            const renameExtension = tile.filePath
              ? splitFileName(fileNameFromPath(tile.filePath)).extension
              : "";
            const noteViewScale =
              tile.type === "note"
                ? normalizedNoteViewScale(tile.noteViewScale)
                : 1;
            const noteCollapsed =
              tile.type === "note" ? Boolean(tile.noteCollapsed) : false;
            const noteCanDecreaseViewScale =
              noteViewScale > NOTE_VIEW_SCALE_MIN + 0.01;
            const noteCanIncreaseViewScale =
              noteViewScale < NOTE_VIEW_SCALE_MAX - 0.01;
            const isTileSelected = selectedTileIdSet.has(tile.id);
            const isTilePrimarySelected =
              isTileSelected &&
              !multiSelectedTiles &&
            ...
            const showFloatingTileChrome =
              tile.type !== "term" &&
              (isRenamingThisTile || isTileSelected || isTilePointerHovered);
            ...
            return (
              <div
                ...
                data-tile-id={tile.id}
                data-terminal-connector-id={
                  tile.type === "term" ? tile.id : undefined
                }
                className={clsx(
                  ...
                  selectedTileId === tile.id &&
                    focusedTerminal?.tileId === tile.id
              ...
              >
                ...
                {tile.type === "term" ? (
                  ...
                  <div
                    className={clsx(
                      ...
                      showFloatingTileChrome
                        ? "-translate-y-[calc(100%+8px)] opacity-100"
                        : "-translate-y-[calc(100%+4px)] opacity-0",
                    )}
                  >
                      ...
                      >
                        <div className="flex items-start gap-2">
                          {tile.type === "note" ? (
                            <HoverTooltip
                              label={
                                noteCollapsed ? "Expand note" : "Collapse note"
                              }
                              placement="bottom"
                            >
                              <button
                                ...
                                className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-[var(--radius-control)] border border-white/10 bg-black/15 text-[var(--text-faint)] transition hover:bg-black/25 hover:text-[var(--text)]"
                                onPointerDown={(event) => {
                                  event.stopPropagation();
                                }}
                                onClick={(event) => {
                                  event.stopPropagation();
                                  toggleNoteCollapsed(tile.id);
                                }}
                              >
                                {noteCollapsed ? (
                                  <CaretRight
                            ...
                            {isRenamingThisTile && isMarkdownTile ? (
                              ...
                              <>
                                <button
                                  ...
                                  title={
                                    isMarkdownTile
                                      ? "Double-click to rename this markdown file"
                                      : tile.title
                                  }
                                >
                                  {tile.title}
                      ...
                      <div className="pointer-events-auto flex shrink-0 items-center gap-1">
                        ...
                        {tile.type === "embed" && tile.embedUrl ? (
                          <button
                            ...
                            onClick={(event) => {
                              event.stopPropagation();
                              void window.collaborator.openExternalUrl(
                                tile.embedUrl as string,
                              );
                            }}
                          >
                            ↗
                ...
                <div className={tileBodyClassName}>
                  {tile.type === "term" ? (
                    ...
                    <DocumentPane
                      ...
                      noteSizingMode={
                        tile.type === "note"
                          ? (tile.noteSizeMode ?? "auto")
                          : undefined
                      }
                      noteViewScale={
                        tile.type === "note" ? noteViewScale : undefined
                      }
                      onNoteContentHeightChange={
                        tile.type === "note"
                          ? (contentHeight) => {
                              handleNoteContentHeightChange(
                                tile.id,
                                contentHeight,
                              );
                            }
                      ...
                      onPrimaryHeadingChange={
                        tile.type === "note"
                          ? (heading) => {
                              void handleNotePrimaryHeadingChange(
                                tile,
                                heading,
                              );
                            }
      ...
      (hoveredConnection.sourceKind === "group"
        ...
        <div
          data-canvas-ui="true"
          className="pointer-events-none absolute inset-0 z-[210]"
        >
          <button
            type="button"
            ...
            style={{
              left:
                hoveredConnection.midpointX * state.viewport.zoom +
                state.viewport.panX,
              top:
                hoveredConnection.midpointY * state.viewport.zoom +
                state.viewport.panY,
            }}
            ...
            onClick={(event) => {
              ...
              if (
                hoveredConnection.sourceKind === "group" &&
                hoveredConnectionSourceGroup
              ) {
                detachContextGroup(
                  hoveredConnectionTerminalTile.id,
                  hoveredConnectionSourceGroup.id,
                );
                return;
              }
              ...
              if (hoveredConnectionSourceTile) {
                detachContextTile(
                  hoveredConnectionTerminalTile.id,
                  hoveredConnectionSourceTile.id,
                );
              }
            }}
          >
            ...
            <div className="flex items-center gap-1.5">
              {DRAW_SIZE_ITEMS.map((item) => {
                ...
                return (
                  <HoverTooltip
                    key={item.size}
                    label={`Pen thickness ${item.label}`}
                  >
                    <button
